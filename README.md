...
| `talon-onboarding` | Maintainer skill that guides onboarding and updating plugins on this marketplace (dual Claude Code + Codex, naming convention, version bumping, PR workflow). | local (`plugins/talon-onboarding`) |
...
`onboard-plugin` skill — it encodes the naming convention, the dual-manifest requirement, the
