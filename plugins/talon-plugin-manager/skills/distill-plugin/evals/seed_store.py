...
Creates, for plugin 'onboard-plugin', a usage record with friction and a ready
marker; for 'terraform-module-steering', an under_trigger record. Point the
distill-plugin skill at the printed store dir to exercise the pipeline without
waiting for real sessions to accumulate.
"""
...
def seed(store_dir: str) -> str:
    for i in range(3):
        append_evidence(store_dir, EvidenceRecord(
            f"onb-{i}", "onboard-plugin", "usage", ["onboard-plugin"],
    ...
    mark_ready(store_dir, "onboard-plugin")
